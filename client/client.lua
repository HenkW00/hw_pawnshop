...
AddEventHandler('hw_pawnshop:sellItem', function(data)
    local data = data
    ...
    if input then
        ...
        if data.quantity < 1 then
            ...
            local done = lib.callback.await('hw_pawnshop:sellItem', 100, data)
            if not done then
                lib.notify({
                    title = 'Error',
...
AddEventHandler('hw_pawnshop:interact', function(data)
